...
const Consent = ({ consents, user }) => {
    ...
    const [selectedConsents, setSelectedConsents] = React.useState(filteredConsents);
    ...
    const handleChange = (event, index, consent) => {
        ...
        fetch(`${appSettings.ApiUri}${updatedConsent.systemIdValue}/${updatedConsent.processing.systemId.identifikatorverdi}/${updatedConsent.active}`, {
            'method': 'PUT',
            'headers': {
    ...
    const createConsent = (event, index, consent) => {
        ...
        setDate(prevDate => [...prevDate, prevDate[index] = dateformatter(new Date())]);
        setYDate(prevDate => [...prevDate, prevDate[index] = date[index]]); 
        
        fetch(`${appSettings.ApiUri}/${consentCreating.processing.systemId.identifikatorverdi}`, {
            'method': 'POST',
            'headers': {
                'Content-type': 'application/json'
            }
                    ...
                    <section className="consent-table">
                        <TableContainer component={Paper}>
                            <Table className={classes.table} aria-label="simple table">
                                <TableHead>
                                    <TableRow>
                                        <TableCell align="center"><strong>Tjeneste</strong></TableCell>
                                        <TableCell align="center"><strong>Personopplysning</strong></TableCell>
                                        <TableCell align="center"><strong>Dato start</strong></TableCell>
                                        <TableCell align="center"><strong>Dato slutt</strong></TableCell>
                                        <TableCell align="center"><strong>Samtykke</strong></TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {consents.map((consent, i) => (
                                        <TableRow key={consent.systemIdValue}>
                                            ...
                                            <TableCell align="center">
                                                ...
                                                {selectedConsents[i] === "true" && consent.expirationDate !== null && yDate[i] === undefined && nDate[i] === undefined && (dateformatter(consent.expirationDate.start))}
                                                {selectedConsents[i] === "true" && yDate[i] !== undefined && (yDate[i])}
                                                {selectedConsents[i] === "false" && consent.expirationDate !== null && yDate[i] === undefined && (dateformatter(consent.expirationDate.start))}
                                                {selectedConsents[i] === "false" && yDate[i] !== null && (yDate[i])}
                                            </TableCell>
                                            <TableCell align="center">
                                                {selectedConsents[i] === "false" && consent.expirationDate !== null && nDate[i] === undefined && (date[i])}
                                                {selectedConsents[i] === "false" && nDate[i] !== null && (nDate[i])}
                                                {selectedConsents[i] === "true" && ("")}
                                            </TableCell>
                                            <TableCell align="center">
                                                {date[i] === appSettings.Untreated && (
                                                    <Button className={classes.root} onClick={(e) => createConsent(e, i, consent)} variant="outlined" color="primary" endIcon={<AddIcon />}>
                                                        Gi samtykke
                                                    </Button>
                                                )}
                                                {date[i] !== appSettings.Untreated && (consent.active === true || consent.active === false) && (
                                                    <RadioGroup row aria-label="consentRadio" name="consent1" value={selectedConsents[i] ? selectedConsents[i] : ""} onChange={(e) => handleChange(e, i, consent)}>
                                                        <FormControlLabel value="true" control={<GreenRadio />} label="Ja" />
